...
* 3 update_mapgen
  * 3.1 specials
    * 3.1.0 "offset_x"
    * 3.1.1 "offset_y"
  * 3.2 overmap tile specification
    * 3.2.0 "assign_mission_target"
    * 3.2.1 "om_terrain"
  * 3.3 mission specials
    * 3.3.0 "target"
...
rows do not have to cover the full 24 rows of an overmap tile.  update_mapgen has a few new fields to support missions, as well as ways to specify which overmap tile will be updated.

# 3.1 specials
update_mapgen has a few new special values.

### 3.1.0 "offset_x"
update_mapgen rows do not have to cover the entire overmap tile.  Rows can be limited to the part
that is actually changing.  The offset field specifies how many rows past 0 to start updating.
- "offset_x" (optional, int) how many rows past 0 to start updating

### 3.1.1 "offset_y"
update_mapgen rows do not have to cover the entire overmap tile.  Rows can be limited to the part
that is actually changing.  The offset field specifies how many columns past 0 to start updating.
- "offset_y" (optional, int) how many columns past 0 to start updating

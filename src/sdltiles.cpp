...
cata::optional<std::pair<tripoint_abs_omt, std::string>> get_mission_arrow(
...
int cata_tiles::get_omt_rotation( std::string &id )
{
    ...
    if( find_tile_looks_like( id, TILE_CATEGORY::C_OVERMAP_TERRAIN ) == nullptr ) {
        //fallback tiles
        id = first_id;
    }
    return rotation;
...
void cata_tiles::draw_om( const point &dest, const tripoint_abs_omt &center_abs_omt, bool blink )
{
    ...
    const cata::optional<std::pair<tripoint_abs_omt, std::string>> mission_arrow =
                get_mission_arrow( overmap_area, center_abs_omt );
    if( mission_arrow ) {

...
mapgen_update_func add_mapgen_update_func( JsonObject &jo )
{
    ...
    if( jo.has_int( "x_offset" ) ) {
        offset.x = jo.get_int( "x_offset" );
    }
    if( jo.has_int( "y_offset" ) ) {
        offset.y = jo.get_int( "y_offset" );
    }
    ...
    const auto update_function = [json_data, offset]( const tripoint & omt_pos, mission * miss ) {
        json_data.update_map( omt_pos, offset.x, offset.y, miss );
    };
